...
// Tools Showcase: infinite carousel slider implementation
// (initialized immediately inside the main DOMContentLoaded handler)
    const showcase = document.querySelector('.tools-showcase');
    if (!showcase) return;

    const track = showcase.querySelector('.tools-carousel');
    if (!track) return;

    // Prepare carousel: clone items to create seamless loop
    const slides = Array.from(track.children);
    if (slides.length === 0) return;

    // Wrap slides in a container if not already wrapped
    track.style.display = 'flex';
    track.style.alignItems = 'stretch';
    track.style.willChange = 'transform';

    // Do NOT duplicate slides here — keep only the original slides (4 total)
    // Wait for images to load inside the carousel so width calculations are accurate
    const waitForImages = () => {
    ...
    // Compute width of the originals
    const computeSetWidth = () => {
        const originals = slides; // original slides array
        return originals.reduce((sum, el) => {
            const style = getComputedStyle(el);
            const mr = parseFloat(style.marginRight || 0);
            return sum + el.getBoundingClientRect().width + mr;
        }, 0);
    };
    ...
    // Speed: pixels per second (lowered for a very slow drift)
    let speed = 14; // ~14 px/sec for gentle movement

    const step = (t) => {
        if (!paused) {
            // delta based movement for frame rate independence
            if (!step.last) step.last = t;
            const delta = (t - step.last) / 1000; // seconds
            step.last = t;
            position += speed * delta;

            // compute maximum scrollable offset (total width - visible area)
            const containerWidth = track.parentElement.getBoundingClientRect().width;
            const maxOffset = Math.max(0, setWidth - containerWidth);

            if (position >= maxOffset) {
                // reset to start (we keep only originals, so this will jump back)
                position = 0;
            }

            track.style.transform = `translateX(-${position}px)`;
        } else {
            // ensure last timestamp is reset to avoid big jumps after hover
            step.last = t;
        }
    ...
    // Pause on hover or focus within the carousel area
    track.addEventListener('mouseenter', () => { paused = true; });
    track.addEventListener('mouseleave', () => { paused = false; });
    track.addEventListener('focusin', () => { paused = true; });
    track.addEventListener('focusout', () => { paused = false; });

    // Prev/Next buttons (nudges)
    ...
    const nudge = (px) => {
        ...
        const containerWidth = track.parentElement.getBoundingClientRect().width;
        const maxOffset = Math.max(0, setWidth - containerWidth);
        position = Math.min(maxOffset, Math.max(0, position + px));
        track.style.transform = `translateX(-${position}px)`;
        // resume after short delay
        clearTimeout(nudge._t);
        nudge._t = setTimeout(() => { paused = false; }, 1200);
    };

    if (prevBtn) prevBtn.addEventListener('click', () => nudge(-220));
    if (nextBtn) nextBtn.addEventListener('click', () => nudge(220));

    // Recompute widths on resize and after images load
    ...
    // Start animation after images have loaded and widths computed
    waitForImages().then(() => {
        setWidth = computeSetWidth();
        rafId = requestAnimationFrame(step);
    }).catch(() => {
    ...
    if (contactForm) {
        contactForm.addEventListener('submit', async (e) => {
            ...
            try {
                const response = await fetch('https://kimn8nceyliz.app.n8n.cloud/webhook/countact', {
                    method: 'POST',
